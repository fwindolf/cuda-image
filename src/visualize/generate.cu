...
__global__ void g_VerticesFromDepth(DevPtr<float4> vertices,
    DevPtr<float> data, const float fx, const float fy, const float cx,
    const float cy)
{
    ...
    float4 n
        = d_normals(make_float3(d, dg.x, dg.y), pos.x, pos.y, fx, fy, cx, cy);
...
__global__ void g_VerticesFromDepthGradients(DevPtr<float4> vertices,
    DevPtr<float3> data, const float fx, const float fy, const float cx,
    const float cy)
{
    const dim3 pos = getPos(blockIdx, blockDim, threadIdx);
...
template <typename T>
__global__ void g_VertexIndicesTriangles(
    DevPtr<uint> indices, const DevPtr<T> data)
{
    const dim3 pos = getPos(blockIdx, blockDim, threadIdx);
    const int idx = getIndex(pos, data.width, data.height);
    ...
    const float cx, const float cy)
{
    ...
    g_VerticesFromDepthGradients<<<grid, block>>>(
 ...
 */
template void cu_VertexIndices(
    DevPtr<uint>& indices, const DevPtr<float>& data);
template void cu_VertexIndices(
    DevPtr<uint>& indices, const DevPtr<float3>& data);

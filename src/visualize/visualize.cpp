...
VisualizerBase::~VisualizerBase() { deinitialize(); }
...
{
    // If run thread is waiting for render request, send one but let it exit bc
    // not running
    running_ = false;
...
std::string VisualizerBase::windowName() const { return name_; }
...
int VisualizerBase::run()
{
    ...
    pangolin::View& d_image = pangolin::Display("image").SetBounds(
        0.f, 1.0f, 0.f, 1.f, (float)w_ / (float)h_);
    ...
    pangolin::RegisterKeyPressCallback(
        pangolin::PANGO_KEY_ESCAPE, [&quit_ref]() { quit_ref = true; });
    ...
    while (running_)
    {
        ...
        // Wait for the show method to produce a render request
        {
            ...
            while (running_ && !renderRequestPending_)
                renderRequested_.wait(
                    lock); // Inside while to counter spurious wakes
        }
        ...
        while (!waitUserAction_ || !quit_)
        {
            ...
            if (uploadPending_)
            {
                ...
                if (!initTexture_())
                {
                    std::cerr << "Could not initialize GlTexture!"
                              << std::endl;
                    uploadPending_ = false;
                    uploadMutex_.unlock();
                    break;
...
bool VisualizerBase::initContext_() { return true; }
...
bool VisualizerBase::bindToTexture_()
{
    ...
    status = cudaGraphicsSubResourceGetMappedArray(
        &array_, texture_.cuda_res, 0, 0);
    if (status != cudaSuccess)
    {
...
{
    auto status = cudaMemcpyToArray(
        array_, 0, 0, uploadData_, uploadDataSize_, cudaMemcpyDeviceToDevice);
    if (status != cudaSuccess)
    {
...
void VisualizerBase::render_() { texture_.RenderToViewportFlipY(); }

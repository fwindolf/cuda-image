...
__global__ void g_Prepare(DevPtr<Q> result, const DevPtr<T> image, Op operation)
{
...
#define DECLARE_PREPARE_OPERATION(type, typeo, operation) \
    template void cu_Prepare(DevPtr<typeo>&, const DevPtr<type>&, operation<type, typeo>);

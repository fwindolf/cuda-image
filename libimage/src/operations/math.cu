#include "image/operations/math_cu.h"
...
template <typename T, typename Op>
void cu_Apply(DevPtr<T> input, Op operation)
{
    dim3 block = block2D(32);
...
struct AddTo
{
    ...
    {
        return [*this] __device__ (T& val1, const T& val2) {
            val1 += val2;
        };
    }
};
...
struct AddValTo
{
    AddValTo(const T& val) : val_(val){}
    
    nvstd::function<void(T&)> __device__ op()
    {
        return [*this] __device__ (T& val) {
            val += val_;
        };
    }
private:
...
struct SubValFrom
{
    SubValFrom(const T& val) : val_(val){}

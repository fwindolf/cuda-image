...
inline bool TypedVisualizer<DEPTH_TYPE>::initContext_()
{
    ...
    if (!success)
        std::cerr << "Ambient Light shader was not compiled correctly or did not link!" << std::endl;
...
inline bool TypedVisualizer<DEPTH_TYPE>::copyToTexture_()
{
    ...
    cu_backProjectAndCalcNormals((float*)uploadData_, (float4*)*vertex_array, w_, h_, fx_, fy_, cx_, cy_);
    ...
    cu_generateVertexIndices((float*)uploadData_, (unsigned int*)*index_array, w_, h_);

...
{   
    ...
    inline __device__ float2 d_gradient_forward(const DevPtr<float>& depth, const int px, const int py)
    {
    ...
    inline __device__ float2 d_gradient_backward(const DevPtr<float>& depth, const int px, const int py)
    {
        const float d = depth(px, py);
    ...
    inline __device__ float2 d_gradient_symmetric(const DevPtr<float>& depth, const int px, const int py)
    {
        const float d = depth(px, py);
    ...
    inline __device__ float3 d_normals(const float d, const float dx, const float dy,
    ...
    inline __device__ float4 d_normals(const float3 d,
                                       const int px, const int py,
                                       const float fx, const float fy, 
                                       const float cx, const float cy)
    {
    ...
    inline __device__ float4 d_backproject(const float d, 
                                           const int px, const int py,
                                           const float fx, const float fy, 
                                           const float cx, const float cy)
    {
    ...
    inline __device__ float3 d_project(const float4 v, 
                                       const float fx, const float fy, 
                                       const float cx, const float cy)
    {

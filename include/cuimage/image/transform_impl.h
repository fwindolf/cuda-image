template <typename T> void Image<T>::setTo(const T& value)
{
...
template <typename T> void Image<T>::replace(const T& value, const T& with)
{
    cu_Replace<T>(*this, value, with);
}

template <typename T> void Image<T>::replaceNan(const T& value)
{
...
template <typename T> T Image<T>::max() const { return cu_Max<T>(*this); }

template <typename T> float Image<T>::sum() const { return cu_Sum<T>(*this); }

template <typename T> float Image<T>::sqSum() const
{
...
template <typename T> float Image<T>::mean() const
{
    // First sum all pixel components and then add.
    Image<float> sum(w_, h_, 0.f);
    cu_PixelSum<T, float>(sum, *this);
    return cu_Sum<float>(sum)
        / (channels<T>() * valid()); // Sum of all components divided by number
                                     // of components
}

template <typename T> float Image<T>::stdDev() const
{
...
template <typename T> unsigned int Image<T>::valid() const
{
    // First mark valid pixels, then count. Makes sure data type storage is
    // sufficient
...
template <typename T> unsigned int Image<T>::nan() const
{
    return size() - valid();
}

template <typename T> unsigned int Image<T>::nonzero() const
{
    Image<int> cnt(w_, h_, 0);
...
template <typename T> float Image<T>::norm1() const
{
    return cu_Norm1<T>(*this);
}

template <typename T> float Image<T>::norm2() const
{
    // This cannot be done as usual reducing operation, so first square, then
    // add
